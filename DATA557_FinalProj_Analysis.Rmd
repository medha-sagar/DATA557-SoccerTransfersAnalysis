...
```{r Load data}
data <- read.csv("/Users/nickw/Downloads/final_dataset.csv")
```
...
```{r Question 1}
summary(lm(points_diff ~ overall_rating+factor(position)+factor(to)+factor(season)+as.numeric(minutes_played),data = data))
```
...
```{r Question 2}
summary(lm(points_diff ~ position:overall_rating+overall_rating+factor(position)+factor(to)+factor(season)+as.numeric(minutes_played), data=data))
```
...
summary(lm(price_num ~ points_diff:overall_rating+points_diff+overall_rating+factor(position)+factor(to)+factor(season)+as.numeric(minutes_played), data=data))
```
